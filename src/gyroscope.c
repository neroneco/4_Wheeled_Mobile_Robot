...
int main(int argc, char **argv){
    ...
    /* Maybe it should be on heap or .data section */
    /* instead of stack:                           */
    struct object gyro;
    struct object acce;
    ...
    //struct i2c_table magneto_table = {
    //                              .file = 0,
    ...
    //i2c_init(&magneto_table);

    /* SETTING UP DEVICES: */
    set_up_LSM6DS33(gyro_acce_table,buffer);
    //set_up_LIS3MDL( magneto_table,  buffer);
    ...
    while(1){
        ...
        //get_data_LIS3MDL( magneto_table,  buffer);
    ...
    /* if I understand it correctly when program recieves SIGINT
     * those lines aren't executed and alloceted memory isn't freed
     */
     // TODO: make SIGING handler so that the memory is freed
